...
const SampleModal: React.FC<SampleModalProps> = ({ currentUrl, lang, onClose, onSelect }) => {
  ...
  const labels: Record<FaceCategory, string> = {
    female: t('sampleModal.categories.female'),
    male: t('sampleModal.categories.male'),
    dog: t('sampleModal.categories.dog'),
    cat: t('sampleModal.categories.cat'),
  };
    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal-content sample-modal" onClick={(event) => event.stopPropagation()}>
        <div className="modal-header">
          <div>
            <h3>{t('sampleModal.title')}</h3>
            <p className="modal-subtitle">{t('sampleModal.subtitle')}</p>
          </div>
          ...
          <div className="sample-grid">
            {samples.map((url, index) => (
              ...
              >
                ...
                />
                <div className="error-placeholder">{t('sampleModal.error')}</div>
              </button>
            ))}
          </div>
        </div>
        <p className="modal-disclaimer">{t('sampleModal.disclaimer')}</p>
      </div>
    </div>
  );
};
