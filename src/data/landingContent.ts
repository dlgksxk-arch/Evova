...
const landingContent: Record<SupportedLandingLanguage, LandingContent> = {
  ko: {
    ...
    intro: {
      title: 'HAMDEVA는 생성 버튼부터 누르게 하는 도구가 아니라, 먼저 이해하고 비교한 뒤 시작하는 콘텐츠형 패션 서비스입니다.',
      paragraphs: [
        'HAMDEVA는 얼굴 사진과 의상 이미지를 조합해 가상 피팅 결과를 만드는 AI 서비스이지만, 화면 구조는 단순 업로드 도구에서 끝나지 않도록 설계되어 있습니다. 사용자는 어떤 사진을 써야 결과가 더 자연스러운지, 어떤 의상이 비교에 적합한지, 전통 의상은 어떤 문화적 맥락과 실루엣을 갖는지까지 읽은 뒤 생성 단계로 이동할 수 있습니다.',
        '이 서비스는 온라인 쇼핑을 하거나 행사 의상을 준비하는 사람, 문화 의상 체험을 고민하는 여행자, 촬영 콘셉트를 정리하는 제작자에게 특히 유용합니다. 실제로 입어보기 전에 분위기와 인상, 실루엣 방향을 빠르게 가늠할 수 있기 때문에 시행착오를 줄이는 데 도움이 됩니다.',
        '또한 HAMDEVA는 전통 의상과 현대 스타일을 같은 플랫폼 안에서 다루기 때문에, 단순히 예쁜 이미지를 만드는 것이 아니라 스타일별 차이를 비교하는 경험을 제공합니다. 한복, 치파오, 기모노, 사리 같은 의상은 구조와 착장 맥락이 다르기 때문에 설명형 콘텐츠와 함께 볼 때 훨씬 이해가 쉬워집니다.',
        '결과적으로 HAMDEVA는 쇼핑 보조 도구이면서도, 패션 리서치와 콘텐츠 기획에 바로 쓸 수 있는 시각 비교 환경입니다. 설명을 먼저 읽고 시작하는 구조 덕분에 생성 결과의 신뢰감도 높아지고, 방문자가 사이트 안에서 머무를 이유도 더 분명해집니다.',
      ],
    },
    features: {
      title: '핵심 기능',
    ...
    sampleInfo: {
      title: '샘플 의상 정보를 먼저 읽으면 더 좋은 결과를 만들 수 있습니다.',
      body: '샘플 의상 정보 페이지에서는 전통 의상과 대표 스타일의 특징, 활용 장면, 가상피팅 포인트를 한 번에 정리해 둡니다. 먼저 읽고 들어오면 어떤 이미지를 고를지 판단하기가 훨씬 쉬워집니다.',
      button: '샘플 의상 정보 보기',
    },
    faq: {
      title: '자주 묻는 질문',
    ...
    modal: {
      ...
      usageTitle: '결과 활용 안내',
      usageItems: ['생성된 결과는 저장해 둘 수 있습니다.', '서로 다른 의상을 비교용 시안으로 확인할 수 있습니다.', '샘플 의상 정보 페이지를 함께 보면 선택이 더 쉬워집니다.'],
      resultTitle: '생성 결과',
      retryButton: '다시 생성하기',
    },
    sampleOutfits: {
      title: '샘플 의상 정보',
      description: '전통 의상과 다양한 스타일별 특징을 확인하고, 가상피팅 전에 어울리는 분위기와 활용 장면을 미리 살펴볼 수 있습니다.',
      introTitle: '의상별 특징을 알고 시작하면 가상피팅의 비교 가치가 훨씬 높아집니다.',
      introParagraphs: [
        '샘플 의상 정보 페이지는 단순히 이미지를 모아둔 갤러리가 아니라, 사용자가 어떤 스타일을 선택해야 할지 판단할 수 있도록 돕는 정보형 페이지입니다. 같은 전통 의상이라도 역사적 배경, 착용 맥락, 실루엣 구조, 장식 포인트가 다르기 때문에, 어떤 분위기를 원하는지 먼저 이해하는 것이 중요합니다.',
        'HAMDEVA는 가상피팅 생성 전에 이 정보를 충분히 읽을 수 있도록 구성했습니다. 사용자는 각 의상 카드에서 개요와 역사, 문화적 사용 맥락, 디자인 특징, 현대 활용 장면, 가상피팅에 적합한 이미지 선택 기준까지 한 번에 확인할 수 있습니다.',
      ],
      catalogTitle: '대표 의상 가이드',
      catalogBody: '아래 의상 카드는 각 스타일의 성격을 빠르게 훑어본 뒤, 바로 가상피팅 비교로 이어질 수 있도록 구성했습니다.',
  ...
  en: {
    ...
    intro: {
      title: 'HAMDEVA is designed as a content-led landing experience first, then a fitting tool when the visitor is ready to generate.',
      paragraphs: [
        'HAMDEVA helps visitors move from uncertainty to a more informed visual decision. Instead of pushing people directly into upload controls, the homepage explains what the service does, why input quality matters, and how different outfit categories can be compared before generation.',
        'This is useful for shoppers, travelers, event planners, stylists, and content creators who want a faster sense of silhouette and mood before buying, renting, or producing something physical. The tool works best when the user understands what kind of look they are testing.',
        'The platform also treats traditional clothing as a subject worth reading about, not just clicking on. Garments such as Hanbok, Qipao, Kimono, and Saree carry design logic and cultural context that make the fitting result more meaningful when the user knows what they are looking at.',
        'That is why HAMDEVA is structured like a service site with editorial guidance. The added context improves trust, makes the landing page more useful, and leads to better try-on decisions when the visitor opens the generator.',
      ],
    },
    features: {
      title: 'Core Features',
    ...
    sampleInfo: {
      title: 'The sample outfit page gives useful context before you generate.',
      body: 'Read about traditional outfits and representative style categories first, then open the generator with a clearer sense of mood, silhouette, and use case.',
    ...
    modal: {
      ...
      actionCardBody: 'The existing login, credit, and generation flow remains the same inside this modal.',
      ...
      usageItems: ['Save the generated result for later review.', 'Compare different outfits before making a decision.', 'Use the sample outfit page as a styling reference.'],
      resultTitle: 'Generated Result',
      retryButton: 'Generate Again',
    },
    sampleOutfits: {
      title: 'Sample Outfit Information',
      description: 'Review traditional outfits and representative style categories before virtual fitting so you can compare mood, context, and use case more clearly.',
      introTitle: 'The better you understand the outfit category, the more useful the fitting result becomes.',
      introParagraphs: [
        'This page is meant to work as an information-rich guide, not just a visual gallery. Each outfit section explains what the garment is, where it comes from, how it is used, and what kind of image reference is most useful before virtual fitting.',
        'That structure helps visitors browse more deliberately. Instead of clicking the first image that looks attractive, they can compare cultural context, silhouette logic, and styling purpose before opening the generator.',
      ],
      catalogTitle: 'Representative Outfit Guides',
      catalogBody: 'Each card below works as a quick reference before you move into the fitting flow.',
  ...
  ja: {
    ...
    intro: {
      title: 'HAMDEVAは、まず内容を読み、そのあと必要なときに生成を始めるランディング型サービスです。',
      paragraphs: [
        'HAMDEVAは、顔写真と衣装画像から試着プレビューを作るAIサービスですが、最初からアップロード操作だけを見せる構成にはしていません。どんな写真が適しているか、どんな衣装が比較しやすいか、伝統衣装にはどんな背景があるかを読んでから試着に進めます。',
        'この流れは、買い物前の比較、イベント衣装の準備、旅行前の文化体験検討、撮影コンセプト整理などに向いています。実際に着る前に雰囲気とシルエットを早く確認できるからです。',
        'また、HAMDEVAは伝統衣装と現代スタイルを同じサービス内で扱うため、単なる画像生成ではなくスタイル比較の体験になります。衣装カテゴリを理解してから生成すると、結果の読み取りもずっと明確になります。',
        'そのためHAMDEVAは、ツール単体ではなく、読み物と体験を組み合わせたサービスとして構成されています。',
      ],
    },
    features: {
      title: '主な機能',
    ...
    sampleInfo: {
      title: 'サンプル衣装情報を先に読むと、試着の比較がもっと分かりやすくなります。',
      body: '伝統衣装や代表スタイルの特徴、使われる場面、試着向きの画像条件を確認してから生成を始められます。',
      button: 'サンプル衣装情報を見る',
    },
    faq: {
      title: 'よくある質問',
    ...
    modal: {
      ...
      usageTitle: '結果の活用',
      usageItems: ['生成結果は保存できます。', '別の衣装との比較に使えます。', 'サンプル衣装情報ページと合わせると選びやすくなります。'],
      resultTitle: '生成結果',
      retryButton: 'もう一度生成',
    },
    sampleOutfits: {
      title: 'サンプル衣装情報',
      description: '伝統衣装や代表スタイルの特徴を確認し、試着前に雰囲気と用途を比較できる情報ページです。',
      introTitle: '衣装カテゴリを理解してから試着すると、結果の比較価値が高まります。',
      introParagraphs: [
        'このページは画像一覧ではなく、衣装の背景と特徴を整理した情報ページです。各衣装について、概要、歴史、文化的な使われ方、デザイン特徴、現代活用、試着時のポイントをまとめています。',
        'そのため、訪問者は見た目の好みだけでなく、どんな雰囲気を試したいのかを考えながら生成に進めます。',
      ],
      catalogTitle: '代表衣装ガイド',
      catalogBody: '各カードを読んでから生成に進むと、比較の軸を持ちやすくなります。',
  ...
  zh: {
    ...
    intro: {
      title: 'HAMDEVA 先是一个内容型落地页，然后才是在需要时打开的生成工具。',
      paragraphs: [
        'HAMDEVA 不希望访客一进入页面就只看到上传控件。首页先说明服务的用途、哪些图片更适合生成、不同服装类型该如何理解，再引导用户进入试衣流程。',
        '这种结构适合网购前比较、活动服装准备、旅行中的文化服饰体验，以及拍摄概念规划。用户可以在真正决定之前，先判断氛围、轮廓和整体印象。',
        '平台把传统服饰与现代风格放在同一套浏览路径里，让用户不仅能生成图片，还能先理解韩服、旗袍、和服、纱丽等服装在结构与语境上的差异。',
        '因此 HAMDEVA 更像“内容 + 工具”的服务，而不是只有一个按钮的单纯生成页面。',
      ],
    },
    features: {
      title: '核心功能',
    ...
    sampleInfo: {
      title: '先阅读示例服装信息，再开始试衣会更容易做出判断。',
      body: '页面会先整理传统服饰和代表风格的特点、使用场景与虚拟试衣要点，帮助你在生成前做出更清晰的选择。',
      button: '查看示例服装信息',
    },
    faq: {
      title: '常见问题',
    ...
    modal: {
      ...
      actionCardBody: '登录、积分和生成逻辑保持不变，只是改为在模态窗口内完成。',
      ...
      usageTitle: '结果可用于',
      usageItems: ['保存生成结果以便后续查看。', '比较不同服装方案。', '结合示例服装信息页一起选择更容易。'],
      resultTitle: '生成结果',
      retryButton: '重新生成',
    },
    sampleOutfits: {
      title: '示例服装信息',
      description: '在虚拟试衣前先了解传统服饰和代表风格的特点、氛围与使用场景。',
      introTitle: '先理解服装类别，再进入试衣流程，比较价值会更高。',
      introParagraphs: [
        '这个页面不是单纯的图片列表，而是带有说明的服装信息页。每种服装都整理了概况、历史、文化场景、设计特点、现代用途和虚拟试衣建议。',
        '这样用户就不会只凭“看起来漂亮”来点击，而是可以先理解自己想测试的风格方向。',
      ],
      catalogTitle: '代表服装指南',
      catalogBody: '阅读每张卡片后再进入生成流程，会更容易建立比较标准。',
